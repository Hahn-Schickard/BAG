...
In this example we are using a configuration which has only one defined service and characteristic. The characteristic will send 5 phase-shifted sine waves as float 32 values (HEX coded) via notification. For using the BAG library and it's DSL we need information of the defined characteristic like the uuid and the data type of the incoming data. So we got the following configuration given for our example:
