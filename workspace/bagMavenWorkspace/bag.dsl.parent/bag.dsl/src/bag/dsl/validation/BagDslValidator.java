 ...
 */
public class BagDslValidator extends AbstractBagDslValidator {
	 ...
	 */
	@Check
	public void macAddressForm(App app) {
		if(app.getMacfilter() != null) {
			for(MacAddress macAddress : app.getMacfilter().getMacaddress()) {
				if(!macAddress.getAddress().matches("^([0-9A-Fa-f]{2}[:]){5}([0-9A-Fa-f]{2})$")) {
					error("MacAddress : '" + macAddress.getAddress() + "' did not match required pattern XX:XX:XX:XX:XX:XX", null, MACADDRESS);
				}
			}
		}
	}
